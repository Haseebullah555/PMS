...
import CreateUserModal from './CreateExtraExpense'
import EditExtraExpenseModal from './EditExtraExpense'

const ExtraExpenseList = () => {
  const {t} = useTranslation()
  const [isModalOpen, setModalOpen] = useState(false)
  const [isEditModalOpen, setEditModalOpen] = useState(false)
  const [selectedExtraExpense, setSelectedExtraExpense] = useState(null)

  const closeModal = () => setModalOpen(false)
  const openModal = () => setModalOpen(true)

  const closeEditModal = () => setEditModalOpen(false)
  const openEditModal = (ExtraExpense: any) => {
    setSelectedExtraExpense(ExtraExpense)
    ...
    <>
          ...
          <div className='card-header cursor-pointer'>
            <div className='card-title m-0'>
              <h3 className='fw-bolder m-0'>
                ...
                {t('global.list', {name: t('extraExpense.extraExpenses')})}
              </h3>
            </div>
            <div>
              <div className='d-none d-lg-flex mt-5'>
                <div className='d-flex align-items-center'>
                  ...
                  >
                    ...
                    {t('global.add', {name: t('extraExpense.extraExpenses')})}
                  </button>
          ...
          <div className='card-body p-9 table-responsive'>
            <DataTable
              ...
              headers={[
                ...
                {
                  headerName: `${t('extraExpense.expenseType')}`,
                  sort: 'expenseType',
                },
                {
                  headerName: `${t('extraExpense.amount')}`,
                  sort: 'amount',
                },
                {
      ...
      {isEditModalOpen && (
        <EditExtraExpenseModal
          ...
          selectedExtraExpense={selectedExtraExpense}
          handleReloadTable={handleReloadTable}
        />
      )}
    </>
