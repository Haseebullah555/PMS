...
const EditPurchaseModal: React.FC<EditPurchaseModalProps> = ({
  ...
  // Populate form with user data when `selectedPurchase` changes
  useEffect(() => {
    if (selectedPurchase) {
      formik.setFieldValue('id', selectedPurchase.id || '')
      formik.setFieldValue('name', selectedPurchase.name || '')
      formik.setFieldValue('phoneNumber', selectedPurchase.phoneNumber || '')
      formik.setFieldValue('address', selectedPurchase.address || '')
    }
  }, [selectedPurchase])
          ...
          <div className='row'>
            ...
            <div className='col-md-6 mb-3'>
              <label className='form-label'>
                {t('purchase.purchase')} <span className='text-danger'>*</span>
              </label>
              <input
                type='text'
              ...
              {formik.touched.name && formik.errors.name && (
                <div className='invalid-feedback'>
                  {t('validation.required', {name: t('purchase.purchase')})}
                </div>
              )}
            </div>
            ...
            <div className='col-md-6 mb-3'>
              ...
              {formik.touched.phoneNumber && formik.errors.phoneNumber && (
                <div className='invalid-feedback'>
                  {t('validation.required', {name: t('Purchase.phoneNumber')})}
                </div>
              )}
            </div>
          </div>
