...
const SignInMethod: React.FC<any> = ({data}) => {
  ...
  useEffect(() => {
    formik1.setFieldValue('userId', data?.user?.id)
    formik1.setFieldValue('newEmail', data?.user?.email)
    formik2.setFieldValue('userId', data?.user?.id)
  }, [data?.id])
  if (!data) {
    return <></>
  }
            ...
            >
              ...
              >
                ...
                <div className='form-text mb-5'>
                  {intl.formatMessage({
                    id: 'SETTING.PASSWORD_MUST_BE_8_CHARACTER_AND_CONTAIN_SYMBOLS',
                  })}
                </div>
