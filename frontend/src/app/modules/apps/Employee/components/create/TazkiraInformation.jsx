...
export default function TazkiraInformation({ handleViewChange, employee_selector }) {
  ...
  const paperValidation = yup.object().shape({
    ...
    tazkira_number: yup
      .string()
      .matches(/^[0-9]+$/, SetLang('Only numbers are allowed'))
      .min(4, SetLang('This field can not be less than 4 chracters'))
      .required(SetLang('This field can not be empty')),
    book_number: yup
      .string()
  ...
  const formik = useFormik({
    initialValues: {
      tazkira_type: employee_selector.tazkira?.tazkira_type,
      tazkira_number: employee_selector?.tazkira?.tazkira_number,
      book_number: employee_selector?.tazkira?.book_number,
      page_number: employee_selector?.tazkira?.page_number,
      registeration_number: employee_selector?.tazkira?.registeration_number,
    },

    validationSchema,
    onSubmit: async (values) => {
      setIsLoading(true)
      dispatch(employeeInformation({ data: values, type: 'tazkira' }))
