...
export default function BankInformation({ handleViewChange, employee_selector }) {
  ...
  const formik = useFormik({
    initialValues: {
      bank_id: employee_selector?.bank?.bank_id,
      account_name: employee_selector?.bank?.account_name,
      account_number: employee_selector?.bank?.account_number,
    },
    ...
    onSubmit: async (values) => {
      setIsLoading(true)
      dispatch(employeeInformation({ data: values, type: 'bank' }))
      setIsLoading(false)
    },
  })
      ...
      <div className='row d-flex mt-4 justify-content-between'>
        ...
        {employee_selector.bank != null && isSubmited != true && (
          <button
            ...
            onClick={() => {
              setIsLoading(true)
              dispatch(postEmployee(employee_selector))
                .then((res) => {
