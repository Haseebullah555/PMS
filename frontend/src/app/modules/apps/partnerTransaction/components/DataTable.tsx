...
import {PartnerTransactionForm} from './_module'
import {useAppDispatch, useAppSelector} from '../../../../../redux/hooks'
import '../../../../../_metronic/assets/css/dataTable.css'
import { getPartnerTransaction } from '../../../../../redux/slices/partnerTransaction/PartnerTransactionSlice'
...
const DataTable: React.FC<any> = ({headers, columns, reload, handleEdit}) => {
  const [data, setData] = useState<PartnerTransactionForm[]>([])
  ...
  const {partnerTransactions} = useAppSelector((state) => state.partnerTransactions)
  ...
  useEffect(() => {
    ...
    dispatch(getPartnerTransaction(params)).then((res) => {
  ...
  useEffect(() => {
    setData(partnerTransactions.data)
    setPagination(partnerTransactions.meta)
  }, [partnerTransactions])
                ...
                {!memoizedLoading &&
                  memoizedData.map((item, index) => (
                    <tr key={index} className='fs-5'>
                      ...
                      <td>{item.partner}</td>
                      <td>{item.amount}</td>
                      <td>{item.type == 0 ? t('global.deposit') : t('global.withdrawal')}</td>
                      <td>{item.date}</td>
