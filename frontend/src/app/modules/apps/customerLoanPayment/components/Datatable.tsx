...
import { CustomerLoanPaymentForm } from './_module'
import { useAppDispatch, useAppSelector } from '../../../../../redux/hooks'
import '../../../../../_metronic/assets/css/dataTable.css'
import { getCustomerWithDetials } from '../../../../../redux/slices/customerLoanPayment/CustomerLoanPaymentSlice'

const SORT_ASC = 'asc'
const SORT_DESC = 'desc'

const DataTable: React.FC<any> = ({ headers, columns, reload, openLoanPaymentModal, openCustomerWithCustomerLoanPaymentModal }) => {
  const [data, setData] = useState<CustomerLoanPaymentForm[]>([])
  ...
  useEffect(() => {
    ...
    dispatch(getCustomerWithDetials(params)).then((res) => {
  ...
  useEffect(() => {
    setData(customersWithDetials?.data ?? null)
    setPagination(customersWithDetials?.meta ?? null)
  }, [customersWithDetials])
    ...
    <div>
      <>
        ...
        <div className='tableFixHead' dir='rtl'>
          <table className='table table-hover table-striped  table-responsive gy-4 gs-5'>
                      ...
                      >
                        <>
                          <Dropdown.Item
                            ...
                            onClick={() => openLoanPaymentModal(item)}
                          >
                          ...
                          <Dropdown.Item
                            as='button'
                            className='fw-bold text-primary'
                            onClick={() => openCustomerWithCustomerLoanPaymentModal(item)}
                          >
                            <i className='fas fa-edit text-primary'></i>
                            <span className='btn btn-sm btn-flex fw-bolder fw-bold text-primary'>
                              {t('customerLoanPayment.payment')}
                            </span>
                          </Dropdown.Item>
                        </>
                      </DropdownButton>
